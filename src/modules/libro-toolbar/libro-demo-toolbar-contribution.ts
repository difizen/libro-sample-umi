...
import { CommandContribution, CommandRegistry, inject, singleton, ToolbarContribution, ToolbarRegistry } from "@difizen/mana-app";
import {
    BellOutlined,
    BulbOutlined,
    HeartOutlined,
...
export class LibroDemoToolbarContribution implements ToolbarContribution, CommandContribution {
    @inject(LibroCommandRegister) protected readonly libroCommand: LibroCommandRegister;

    registerCommands(command: CommandRegistry): void {
        this.libroCommand.registerLibroCommand(command, LibroDemoToolbarCommand['demoToolbarCommand1'], {
        ...
        this.libroCommand.registerLibroCommand(command, LibroDemoToolbarCommand['demoToolbarCommand2'], {
            execute: async (cell, libro, path) => {
        ...
        this.libroCommand.registerLibroCommand(command, LibroDemoToolbarCommand['demoToolbarCommand3'], {
            execute: async (cell, libro, path) => {
        ...
        this.libroCommand.registerLibroCommand(command, LibroDemoToolbarCommand['demoToolbarCommand4'], {
            execute: async (cell, libro, path) => {
                console.log('工具栏按钮注册在顶部右侧工具栏和 cell 右侧工具栏，并且成组示例')
            },
            isEnabled: () => {

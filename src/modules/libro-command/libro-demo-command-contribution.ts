...
import { CommandContribution, CommandRegistry, inject, singleton } from "@difizen/mana-app";
    ...
    registerCommands(command: CommandRegistry): void {
        this.libroCommand.registerLibroCommand(command, LibroDemoCommand['demoCommand1'], {
            execute: async (cell, libro, path) => {
                console.log('使用 LibroCommandRegister 的方式注册的 demoCommand1 被执行',cell, libro, path)
            },
            isEnabled: () => {
                return true
            },
        ...
        command.registerCommand(LibroDemoCommand['demoCommand2'],{
            execute: async (args1,args2) => {
                console.log('使用 CommandRegistry 的方式注册的 demoCommand2 被执行',args1,args2)
            },
        })
    }
}
